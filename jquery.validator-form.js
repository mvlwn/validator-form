...
(function($) {
  ...
  ValidatorForm.prototype = {
    ...
    collectElements: function() {
      ...
      $.each(input, function(name, options) {
        ...
        if (object.length == 0) {
          ...
          // console.log("No objects found for: " + name);
        } else {
    ...
    collectGroups: function() {
      ...
      $.each(input, function(name, options) {
        if (options["group"] !== undefined) {
          ...
          if (!form.getGroup(groupName)) var group = new ValidatorGroup(groupName, form);
          groups.push(group);
        }
      });
      return groups;
    ...
    setupInputFromElements: function() {
      var form = this;
      form.currentForm.find("[" + form.settings.validationAttribute + "]")
        .each(function() {
        var object = $(this);
        var input = {};
        $(this.attributes)
          .each(function(i, attribute) {
          if (new RegExp("data-", "")
            .test(attribute.name)) input[attribute.name.replace("data-", "")] = attribute.value;
        });
    ...
    displayErrorlist: function(element, errors) {
      if (this.settings.displayErrorlist) return this.settings.displayErrorlist(element, errors);
      return this.methods.displayErrorlist(element, errors);
    },

    displayValidationSuccess: function(element) {
      if (this.settings.displayValidationSuccess) return this.settings.displayValidationSuccess(element);
      return this.methods.displayValidationSuccess(element);
    },

    errorlist: function(element, errors) {
      if (this.settings.errorlist) return this.settings.errorlist(element, errors);
      return this.methods.errorlist(element, errors);
    },

    cleanElement: function(element) {
      if (this.settings.cleanElement) return this.settings.cleanElement(element);
      return this.methods.cleanElement(element);
    },

    container: function(element) {
      if (this.settings.container) return this.settings.container(element);
    ...
    methods: {
      ...
      cleanElement: function(element) {
        ...
        element.container.removeClass(errorContainerClass)
          .removeClass(validClass);
        element.container.find("." + errorlistClass)
          .remove();
      },
  ...
  ValidatorElement = function(object, form) {
    ...
    this.group = function() {
      return form.getGroup(this.input.group)
    };
  ...
  ValidatorElement.prototype = {
    ...
    attach: function(event) {
      ...
      if (event == "change") {
        element.object.bind("change", function(e) {
          ...
          if (group) group.callEvent("change");
    ...
    validate: function(e) {
      ...
      if (group) group.callEvent((element.valid ? "valid" : "invalid"));
  ...
  ValidatorGroup = function(name, form, options) {
    ...
    if (form.settings.group) $.extend(this.events, form.settings.group["all"], form.settings.group[name]);
  };
  ...
  ValidatorGroup.prototype = {
    ...
    callEvent: function(event) {
      if (this.events[event]) this.events.call(event);
    },
    elementsByGroupName: function() {
      var group = this;
      var elements = [];
      $.each(group.form.elements, function(i, element) {
        if (element.group == group.name) $(elements)
          .push(element);
      });
      return elements;
    }
  ...
  $.validator.addRule("format", $.rule({
    ...
    errorMessage: function() {
      return $.validator.messages.format
    }
  }));
